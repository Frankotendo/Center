                    ...
                    {activeTab === 'arsenal' && (
                        <div className="grid grid-cols-2 gap-2">
                            {SECURITY_TOOLS.map(tool => (
                                <div 
                                    key={tool.id} 
                                    onClick={() => { setSelectedTool(tool); onInput(tool.command); }} 
                                    className={`p-2 bg-[#1f1f1f] border rounded-lg cursor-pointer transition-all active:scale-95 ${selectedTool?.id === tool.id ? 'border-rose-500 bg-[#252525]' : 'border-[#333] hover:border-gray-500'}`}
                                >
                                    <i className={`fas ${tool.icon} text-lg mb-1 ${selectedTool?.id === tool.id ? 'text-rose-500' : 'text-gray-500'}`}></i>
