...
const MANUAL_MISSIONS: Mission[] = [
  {
    ...
    briefing: "Comrade, uplink established. Target identified as a rogue Vercel node. We need a full port scan to assess vector surface. Execute nmap scan immediately. No sleeping!"
  },
  {
    id: 'm2',
    ...
    briefing: "New intelligence my guy. Supabase database node detected with anomalous traffic. Suspected misconfiguration. Probe the PostgreSQL port for access. Sharp sharp."
  },
  {
    id: 'm3',
    ...
    briefing: "We are tracking a target utilizing 'Starbucks_Public_WiFi' for sensitive comms. Switch interface to monitor mode and capture the handshake. Don't dull."
  },
  {
    id: 'm4',
...
const generateCurriculum = (): Mission[] => {
  ...
  const tracks = [
    ...
    { 
      code: 'CRYPTO', 
      name: 'CRYPTO_BLACK_OPS', 
      tools: ['mythril', 'slither', 'cast', 'foundry', 'ganache', 'metamask-cli'],
      templates: [
        { title: 'REENTRANCY', desc: 'Drain the staking pool via recursive fallback calls.' },
        { title: 'WALLET DRAIN', desc: 'Reverse engineer signed txn to recover private key.' },
        { title: 'FLASH LOAN', desc: 'Manipulate oracle price feed using a flash loan attack.' },
        { title: 'RUG PULL', desc: 'Audit a suspicious token contract for hidden mint functions.' },
        { title: 'BRIDGE HACK', desc: 'Bypass signature verification on the cross-chain bridge.' }
      ]
    }
  ];
...
const App: React.FC = () => {
  ...
  const handleSelectMission = async (id: string) => {
    ...
    if (!target) {
        ...
        const typeHint = mission.title.includes('WIFI') ? 'WiFi Network' : 
                         ...
                         mission.title.includes('IOT') ? 'IoT Device' : 
                         mission.title.includes('CRYPTO') ? 'Smart Contract / Blockchain Wallet' : 'Server';
  ...
  // Helper for generating missions
  const triggerMissionGeneration = async (topic?: string) => {
      setGameState(prev => ({
          ...prev,
          isProcessing: true,
      }));
      ...
      try {
          const newMissions = await generateNewMissions(completedCount, topic);
          ...
          setGameState(prev => ({
              ...
              terminalHistory: [
                  ...prev.terminalHistory,
                  { type: 'system', content: `\n// UPLINK SUCCESSFUL.\n// ${processedMissions.length} NEW ${topic ? topic.toUpperCase() + ' ' : ''}OPERATIONS GENERATED.\n// CHECK MISSION HUB.`, timestamp: Date.now() }
              ]
          }));
      } catch (error) {
            setGameState(prev => ({
  ...
  const handleCommand = async (cmd: string) => {
    ...
    if (cmd === 'generate missions' || cmd === 'gen ops') {
        ...
        await triggerMissionGeneration();
        return;
    }
    ...
    // Ensure mission is selected IF executing a tool command, but allow "chat" without mission
    // We roughly guess if it's a tool by looking for known tools or if a mission is selected.
    // However, executeCommand now handles chat, so we ALWAYS let it through, 
    // but pass a dummy mission if none selected for context (or just fail gracefully in prompt if no mission).
    // The current executeCommand expects a mission.
    
    let mission = gameState.currentMissionId ? gameState.missions.find(m => m.id === gameState.currentMissionId)! : null;
    let target = mission ? gameState.targets[mission.targetId] : null;

    if (!mission) {
         // Create a dummy context for "Lobby" chat
         mission = { id: 'lobby', title: 'NO ACTIVE MISSION', difficulty: 'Beginner', description: 'User is in the lobby.', objectives: [], targetId: 'none', completed: false, recommendedTools: [], briefing: '' };
         target = { id: 'none', name: 'N/A', type: TargetType.SERVER, ip: '0.0.0.0', os: 'None', vulnerabilities: [], ports: [], status: 'online', description: 'No target selected' };
    }
    ...
    // Artificial Latency only for tools
    const isTool = cmd.includes('nmap') || cmd.includes('scan') || cmd.includes('hydra');
    if (isTool) await wait(1500);
    ...
    if (result.targetUpdate && mission && mission.id !== 'lobby') {
    ...
    // Handle System Actions (AI Controlled)
    if (result.systemAction === 'generate_missions') {
        // AI decided to load missions
        await triggerMissionGeneration(result.suggestedMissionTopic);
    } else if (didCompleteMission) {
        // Auto-generate on completion
        const completedCount = newMissions.filter(m => m.completed).length;
        // Basic generation on completion (no topic focus)
  ...
  const handleVoiceCommand = (text: string) => {
    // Inject voice command into terminal as a secure transmission
    setGameState(prev => ({
        ...prev,
        terminalHistory: [...prev.terminalHistory, { type: 'system', content: `\n// SECURE VOICE CHANNEL: "${text}"`, timestamp: Date.now() }]
    }));
    handleCommand(text);
  };
