...
export const generateSpeech = async (text: string): Promise<ArrayBuffer | null> => {
  try {
    ...
    const cleanText = text.length > MAX_LENGTH ? text.substring(0, MAX_LENGTH) + "..." : text;
...
export const generateLecture = async (topic: string): Promise<Lecture> => {
    const prompt = `
        ...
        FOCUS: If the topic allows, relate it to Blockchain, Cryptography, or Financial Security.
